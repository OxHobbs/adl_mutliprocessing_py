# Requires environment variables for STORAGE_ACCOUNT_NAME and STORAGE_ACCOUNT_KEY
import os
...
def upload_file_to_directory(file_path: str, directory_client) -> str:
    try:
        global file_system_client
        # global directory_client
...
def main() -> None:
    sa_name = os.environ.get('STORAGE_ACCOUNT_NAME')
    sa_key = os.environ.get('STORAGE_ACCOUNT_KEY')
    ...
    for file in files_to_upload:
        ...
        if file_already_exists:
            print(f"{file_name} already exists in ADL at {directory}")
        else:
            print(f"{file_name} DOES NOT already exist in ADL at {directory}")
            files_non_existent.append(file)

    if files_non_existent:
        pool = mp.Pool(processes=10)
        results = [pool.apply_async(upload_file_to_directory, args=(file, directory_client)) for file in
                   files_non_existent]
        output = [r.get() for r in results]
        print(output)
    else:
        print("No files that need to be uploaded")
